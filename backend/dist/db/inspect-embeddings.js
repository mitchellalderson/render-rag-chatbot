...
exports.inspectEmbeddings = inspectEmbeddings;
...
async function inspectEmbeddings() {
    ...
    try {
        // Get total count
        const totalResult = await config_1.pool.query('SELECT COUNT(*) FROM documents');
        const totalCount = parseInt(totalResult.rows[0].count, 10);
        console.log(`📊 Total documents: ${totalCount}`);
        // Count documents with NULL embeddings
        const nullResult = await config_1.pool.query('SELECT COUNT(*) FROM documents WHERE embedding IS NULL');
        ...
        const nonNullResult = await config_1.pool.query('SELECT COUNT(*) FROM documents WHERE embedding IS NOT NULL');
        ...
        if (nonNullCount > 0) {
            ...
            const sampleResult = await config_1.pool.query(`
        SELECT 
          id,
      ...
      `);
        ...
        if (nonNullCount > 0) {
            ...
            const testResult = await config_1.pool.query(`
        SELECT 
          id,
...
//# sourceMappingURL=inspect-embeddings.js.map